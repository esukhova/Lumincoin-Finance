...
export class CommonUtils {
    static async getSelectCategory(operation, openNewRoute) {

        let response = null;
        if (operation.type === 'expense') {
            response = await CategoriesService.getCategories('expense');
        } else {
            response = await CategoriesService.getCategories('income');
        }
        ...
        for (let i = 0; i < selectCategoryElement.options.length; i++) {
            if (selectCategoryElement.options[i].innerText === operation.category) {
